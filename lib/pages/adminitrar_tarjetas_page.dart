...
class AdministrarTarjetasPage extends StatefulWidget {
  @override
  _AdministrarTarjetasPageState createState() =>
      _AdministrarTarjetasPageState();
}
...
class _AdministrarTarjetasPageState extends State<AdministrarTarjetasPage> {
  ...
  Widget build(BuildContext context) {
    final provider =
        Provider.of<TarjetasCreditoProvider>(context, listen: true);
    ...
    return Scaffold(
          ...
          IconButton(
            ...
            onPressed: () async {
              ...
              if (resultado > 0) {
                ...
                mostrarSnackBar(
                    context, 'No tiene ninguna tarjeta para eliminar');
              }
            },
          ),
        ],
      ...
      body: (provider.tarjetas[0] == null)
          ? Center(
              child: Text('No tiene ninguna tarjeta agregada'),
            )
          : Column(
              children: [
                Padding(
                    ...
                    itemBuilder: (context, index) {
                      return Dismissible(
                        ...
                        onDismissed: (DismissDirection direction) async {
                          await DBService.db
                              .deleteTarjeta(provider.tarjetas[index].id);
                        ...
                        child: Card(
                          child: ListTile(
                            ...
                            subtitle:
                                Text(provider.tarjetas[index].cardHolderName),
                            title: Text('**** **** **** ' +
                                provider.tarjetas[index].cardNumber.substring(
                                    provider.tarjetas[index].cardNumber.length -
                                        4)),
                            trailing: Icon(
