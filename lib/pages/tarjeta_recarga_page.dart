          ...
          child: Column(
            ...
            children: [
              ...
              (provider.tipoPago == 3)
                  ? Column(
                      children: [
                        Text("Recarga de tarjeta para: ",
                            style: TextStyle(
                                fontWeight: FontWeight.bold, fontSize: 14.0)),
                        Text(provider.dependiente,
              ...
              GestureDetector(
                onTap: () {
                  Provider.of<TarjetasCreditoProvider>(context, listen: false)
                ...
                builder: (_, AsyncSnapshot<List<TarjetaCredito>> snapshot) {
                  if (snapshot.hasData) {
                    if (snapshot.data[0] == null) {
                      return Center(
                        child: Text('No tiene tarjetas registradas'),
                      );
                    }
    ...
    return Container(
      ...
      decoration: BoxDecoration(
          image: DecorationImage(
              image: AssetImage('assets/images/tarjeta_consumo2.jpg'),
              fit: BoxFit.cover)),
    );
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    return TextButton(
      ...
      style: TextButton.styleFrom(
          ...
          primary:
              (provider.optRecarga == this.opt) ? Colors.white : Colors.black,
          backgroundColor: (provider.optRecarga == this.opt)
              ? Color(0xff00472B)
              : Color(0xffEBEBEB)),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return ElevatedButton(
      onPressed: (provider.tarjetaSeleccionada == 0)
          ? null
          : () async {
              if (provider.tarjetaSeleccionada == 0)
                return mostrarSnackBar(
                    context, 'Seleccione una tarjeta para continuar');
              //await DBService.db.deleteAll();
              if (providerB.montoRecarga.isEmpty ||
                  providerB.montoRecarga == '00.00')
                return mostrarSnackBar(
                    context, 'Seleccione o ingrese un monto');

              if (!formValidator.isNumeric(providerB.montoRecarga))
                return mostrarSnackBar(context, 'Ingrese un monto valido');
              ...
              Navigator.pushNamed(context, 'detalle_recarga',
                  arguments: tarjeta.first);
            },
