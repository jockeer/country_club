...
class ContactService {
  ...
  Future<dynamic> obtenerContacto() async {
    final url = Uri.parse(
        '${constantes.dominio}/laspalmas/ste/api-v1/services/get_contact_us');
    //print(url);
  ...
  Future<dynamic> enviarMensaje(String mensaje) async {
    final url = Uri.parse(
        '${constantes.dominio}/laspalmas/ste/api-v1/services/message_contact_us_customer');
    final conexion = await comprobarInternet();
    if (!conexion) {
      return null;
    }
    try {
      final respuesta = await http
          .post(url, body: {"message": mensaje, "access_token": prefs.token});
